...
with DAG('micromobility_hourly_ingestion', start_date=datetime(2025, 1, 1), schedule='@hourly', catchup=False) as dag_hourly:
    ...
    # 1. BRONZE LAYER: Parallel Extractionimport sys
import os
from datetime import datetime
from airflow import DAG
try:
...
with DAG('micromobility_hourly_ingestion', start_date=datetime(2025, 1, 1), schedule='@hourly', catchup=False) as dag_hourly:
        ...
        for provider in PROVIDERS:
            ...
            silver_lanes[provider] = DbtTaskGroup(
                ...
                render_config=RenderConfig(
                    ...
                    # FIX: Adds tests immediately after each staging view is built
    ...
    gold = DbtTaskGroup(
        ...
        profile_config=profile_cfg,
        render_config=RenderConfig(
            # Selects the final models AND the seeds
            select=["path:models/marts", "path:seeds"],
            load_method=LoadMode.DBT_MANIFEST,
            # FIX: Runs dbt build sequence (Seeds -> Models -> Tests)
            test_behavior=TestBehavior.AFTER_EACH
        )
    )
